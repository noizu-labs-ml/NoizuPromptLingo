...
if System.get_env("PHX_SERVER") do
  config :the_robot_wars, TheRobotWarsWeb.Endpoint, server: true
end

config :the_robot_wars, :redis,
...
if config_env() == :prod do
  ...
  config :the_robot_wars, :dns_cluster_query, System.get_env("DNS_CLUSTER_QUERY")
  ...
  config :the_robot_wars, :frontend_url, System.get_env("FRONTEND_URL")
  ...
  if oidc_client_id = System.get_env("OIDC_CLIENT_ID") do
    ...
    config :the_robot_wars, :oidc_enabled, true
  end
  ...
  if saml_metadata = System.get_env("SAML_IDP_METADATA_URL") do
    ...
    config :the_robot_wars, :saml_enabled, true
  end

  # ── SSO: Social OAuth (each enabled when *_CLIENT_ID is set) ──
  config :the_robot_wars, :sso_require_invite, System.get_env("SSO_REQUIRE_INVITE") == "true"
  ...
  oauth_providers =
    if google_id = System.get_env("GOOGLE_CLIENT_ID") do
      ...
      config :the_robot_wars, :google_enabled, true
  ...
  oauth_providers =
    if fb_id = System.get_env("FACEBOOK_CLIENT_ID") do
      ...
      config :the_robot_wars, :facebook_enabled, true
  ...
  oauth_providers =
    if gh_id = System.get_env("GITHUB_CLIENT_ID") do
      ...
      config :the_robot_wars, :github_enabled, true
  ...
  #     config :the_robot_wars, :linkedin_enabled, true

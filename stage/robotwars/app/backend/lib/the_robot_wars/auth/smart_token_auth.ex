defmodule TheRobotWars.Auth.SmartTokenAuth do
    ...
    case find_active_user_by_email(email) do
      {:ok, user} ->
        ...
        case find_and_authorize_otp(user_ref, otp_code, {:auth, :password_reset}, conn, context) do
          {:ok, _token} ->
            TheRobotWars.Users.Credentials.update_password(user, new_password, context)
    ...
    case SmartToken.authorize!(token_key, %{}, context) do
      {:ok, token} ->
        with {:ok, user} <- Noizu.EntityReference.Protocol.entity(token.resource, context) do
          ...
          |> TheRobotWars.Repo.update_all(set: [verified: true])
  ...
  defp lookup_otp_token(user_ref, otp_code, scope) do
    ...
    tokens = TheRobotWars.Repo.all(q)
  ...
  defp find_active_user_by_email(email) do
    ...
    case TheRobotWars.Repo.one(q) do
      ...
      record ->
        settings = Noizu.Entity.Meta.persistence(TheRobotWars.Users.User) |> hd
        TheRobotWars.Users.User.from_record(record, settings, Noizu.Context.system(), [])
    end
  end
end

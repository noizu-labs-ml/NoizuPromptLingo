defmodule TheRobotWars.Media do
  alias TheRobotWars.Media.Asset, as: Entity
  alias TheRobotWars.Schema.Media.Asset, as: Schema
  use Noizu.Repo
  ...
  def get_cached_variant(media_id, canonical_params) do
    import Ecto.Query
    TheRobotWars.Repo.one(
      from v in TheRobotWars.Schema.Media.Variant,
        where: v.media_id == ^media_id and v.params == ^canonical_params
    )
  end

  def cache_variant(attrs) do
    %TheRobotWars.Schema.Media.Variant{}
    |> TheRobotWars.Schema.Media.Variant.changeset(attrs)
    |> TheRobotWars.Repo.insert(on_conflict: :nothing)
  end

  def fetch_from_s3(key) do
    config = Application.get_env(:the_robot_wars, TheRobotWars.Storage, [])
  ...
  def get_or_create_variant(media, transform_params) do
    canonical = TheRobotWars.Media.Transform.canonical_params(transform_params)
      ...
      nil ->
        with {:ok, original_binary} <- fetch_from_s3(media.file),
             {:ok, transformed, content_type} <-
               TheRobotWars.Media.Transform.transform(original_binary, transform_params) do
          variant_key = TheRobotWars.Media.Transform.variant_s3_key(media.file, transform_params)

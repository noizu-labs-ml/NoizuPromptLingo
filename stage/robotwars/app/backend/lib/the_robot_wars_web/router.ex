defmodule TheRobotWarsWeb.Router do
  use TheRobotWarsWeb, :router
  ...
  pipeline :authenticated do
    plug TheRobotWarsWeb.AuthPipeline
  end
  ...
  pipeline :rate_limited_auth do
    plug TheRobotWarsWeb.Plugs.RateLimit, action: :auth
  end

  pipeline :rate_limited_sensitive do
    plug TheRobotWarsWeb.Plugs.RateLimit, action: :auth_sensitive
  end

  pipeline :org_viewer do
    plug TheRobotWarsWeb.Plugs.RequireRole, role: "viewer"
  end

  pipeline :org_editor do
    plug TheRobotWarsWeb.Plugs.RequireRole, role: "editor"
  end

  pipeline :org_admin do
    plug TheRobotWarsWeb.Plugs.RequireRole, role: "admin"
  end

  pipeline :org_owner do
    plug TheRobotWarsWeb.Plugs.RequireRole, role: "owner"
  end

  pipeline :admin do
    plug TheRobotWarsWeb.Plugs.RequireAdmin
  end

  scope "/", TheRobotWarsWeb do
    pipe_through :api
    get "/health", HealthController, :index
  end

  scope "/api/v1", TheRobotWarsWeb do
  ...
  scope "/media", TheRobotWarsWeb do
    pipe_through [:api]
  ...
  if Application.compile_env(:the_robot_wars, :saml_enabled) do
    scope "/sso/saml" do
  ...
  # OIDC redirect flow
  scope "/auth/oidc", TheRobotWarsWeb do
    pipe_through [:sso_session]
  ...
  if Application.compile_env(:the_robot_wars, :dev_routes) do
    import Phoenix.LiveDashboard.Router

    scope "/dev" do
      pipe_through [:fetch_session, :protect_from_forgery]
      live_dashboard "/dashboard", metrics: TheRobotWarsWeb.Telemetry
    end
  end
end

defmodule TheRobotWarsWeb.MediaServeController do
  use TheRobotWarsWeb, :controller

  alias TheRobotWars.Media
  alias TheRobotWars.Media.Transform
  ...
  defp serve_original(conn, media) do
    if media.visibility == "public" do
      url = TheRobotWars.Storage.presigned_download_url(media.file)
  ...
  defp serve_transformed(conn, media, transform_params) do
    case Media.get_or_create_variant(media, transform_params) do
      {:ok, variant_key, content_type} ->
        if media.visibility == "public" do
          url = TheRobotWars.Storage.presigned_download_url(variant_key)
  ...
  defp get_user_id(conn) do
    try do
      case Plug.Conn.get_req_header(conn, "authorization") do
        ["Bearer " <> token] ->
          case TheRobotWars.Guardian.decode_and_verify(token) do
            {:ok, claims} ->
              case TheRobotWars.Guardian.resource_from_claims(claims) do
                {:ok, %TheRobotWars.Users.Sessions.UserSession{user: {:ref, _, id}}} -> id
                {:ok, %TheRobotWars.Users.Sessions.UserSession{user: %{id: id}}} -> id
                _ -> nil
              end

...
> Step-by-step methodology for building a client-specific style guide from the six design system specifications. Covers pure styles and mixed (80/20) ratios.
...
> **Rendering target:** The completed markdown style guide feeds the styleguide-engine via YAML conversion. See `outputs/engine-styleguide.md` for the full extraction workflow.
...
Follow these steps sequentially. Steps 1–4 build the **foundation** (sections 00–04). Steps 5–8 build **components** (sections 05–12) — these can be parallelized. Steps 9–11 cover **cross-cutting concerns** and validation. Each step produces a numbered section of the final style guide matching the engine's section registry.
...
**Output:** Design philosophy + brand identity.

**Engine output:** Extracts to `style-guide.meta.yaml` (slug, name, title, description) + `branding.yaml` (intent, perception, audience, tone, keywords). See [`engine-styleguide.md §4.1`](../outputs/engine-styleguide.md#41-00-header--brand-identity--style-guidemetayaml--brandingyaml).
...
**Output:** Complete CSS custom properties block, organized by category.

**Engine output:** Decomposes into `style-guide.vars.yaml` groups. Each `/* Category */` comment becomes a named group. Only include tokens that differ from the engine's base theme — the cascade derives ~300 tokens from ~12 seeds. See [`engine-styleguide.md §4.2`](../outputs/engine-styleguide.md#42-01-design-tokens--style-guidevarsyaml).
...
**Output:** Palette visualization + usage rules + contrast table.

**Engine output:** Extracts to `style-guide.color-palette.yaml` (color groups with `var()` references + usage notes). If light/dark modes are defined, also `style-guide.color-modes.yaml`. See [`engine-styleguide.md §4.3`](../outputs/engine-styleguide.md#43-02-color-palette--style-guidecolor-paletteyaml).
...
**Output:** Font stack declarations + type scale table + font source table with links.

**Engine output:** Extracts to `style-guide.typography.yaml` — font family entries (`typography[]`) with var references, weights, descriptions; type scale becomes `typography-classes[]`. Font URL goes into `branding.yaml`. See [`engine-styleguide.md §4.4`](../outputs/engine-styleguide.md#44-03-typography--style-guidetypographyyaml).
...
**Output:** Spacing scale + component spacing + grid specification + layout patterns.

**Engine output:** Extracts to `style-guide.spacing.yaml` (grid, container, rhythm) + `style-guide.page-layouts.yaml` (content width presets with chrome). See [`engine-styleguide.md §4.5`](../outputs/engine-styleguide.md#45-04-spacing--layout--style-guidespacingyaml--style-guidepage-layoutsyaml).
...
**Output:** CSS snippets for buttons and inputs as separate sections.

**Engine output:** Component CSS goes into `style-guide.css-snippets.yaml` as named snippets with `target-section: buttons` or `target-section: inputs`. Snippets are accumulated (merged with base theme). See [`engine-styleguide.md §4.6`](../outputs/engine-styleguide.md#46-05-06-buttons--inputs--style-guidecss-snippetsyaml).
...
**Output:** Three separate sections (navigation, status indicators, cards).

**Engine output:** Navigation → `style-guide.shell-layouts.yaml` (full-page shells with chrome + zones). Status indicators → `style-guide.semantic-classes.yaml` (danger, warning, info, success classes). Cards → `style-guide.css-snippets.yaml` (card CSS as snippets with `target-section: cards`). See [`engine-styleguide.md §4.7-4.8`](../outputs/engine-styleguide.md#47-07-navigation--style-guideshell-layoutsyaml).
...
**Output:** Three separate sections (core components, project components, screens).

**Engine output:** Core components inherit from base theme. Project components → `style-guide.css-snippets.yaml` (custom component CSS). Screens are rendered by the engine's section system — no additional YAML needed unless custom page-sections are defined.
...
> **Engine output:** Optional — advanced CSS snippets or design-sections. Base theme provides defaults.
...
2. For each image asset, include a **structured asset prompt payload** (see [`skills/shared/asset-prompt-payload-schema.md`](../../../../shared/asset-prompt-payload-schema.md)) with:
   - `prompt` — generation text, negative prompt, style slug, per-tool hints
   - `requirements` — dimensions, format, transparency, color space, DPI, product targets
   - `eval` — weighted scoring criteria for automated grading/approval of generated images
3. Define **iconography conventions** — style, stroke weight, size, source library
4. Define **illustration approach** (if applicable)
5. Document **logo usage rules** (if not covered in §00)

**Example asset payload for a hero image:**
```yaml
- id: hero-team-collab-001
  prompt:
    text: "diverse engineering team reviewing dashboard on large monitor, natural office lighting, warm tones, shallow depth of field, over-the-shoulder angle"
    negative: "staged, stock photo feel, empty office, cold lighting"
    style: photography
    tool_hints:
      dalle3:
        quality: hd
  ...
  eval:
    ...
    criteria:
      relevance:
        weight: 0.25
        scale: [1, 5]
        description: "Shows authentic team collaboration with technology"
        fail_signals:
          - "empty or staged feel"
          - "wrong subject matter"
      composition:
        weight: 0.15
        scale: [1, 5]
        description: "Clear focal point, rule of thirds, depth"
      technical:
        weight: 0.15
        scale: [1, 5]
        description: "Sharp, correct white balance, no artifacts"
        fail_signals:
          - "blurry"
          - "color cast"
      web_ready:
        weight: 0.20
        scale: [1, 5]
        description: "Correct dimensions, has text overlay space, compresses well"
        fail_signals:
          - "no clear text placement area"
          - "too busy for overlay"
      brand_fit:
        weight: 0.10
        scale: [1, 5]
        description: "Warm, approachable, matches brand color palette"
      stopping_power:
        weight: 0.15
        scale: [1, 5]
        description: "Genuine feel, would click through to learn more"
    reject_if:
      - "obvious AI artifacts (hands, faces)"
      - "watermarks"
      - "identifiable copyrighted content"
```

**Output:** Asset guidelines with search-ready image descriptions and structured eval payloads.
...
Every completed style guide should follow this canonical structure. Sections are numbered to match the styleguide-engine's section registry, ensuring the markdown source and the engine rendering stay in sync. The markdown is the **source of truth** — extract YAML from it via [`engine-styleguide.md`](../outputs/engine-styleguide.md).

Sections marked *(stub if not applicable)* may be a single line noting "Not applicable — [reason]" or "See [Component Styling](#component-styling) for [topic]." They must still appear as headings so the engine rendering has a 1:1 mapping.
...
<!-- Engine: branding.yaml + style-guide.meta.yaml -->
...
<!-- Engine: style-guide.color-palette.yaml + style-guide.color-modes.yaml -->
...
<!-- Engine: style-guide.typography.yaml -->
...
<!-- Engine: style-guide.spacing.yaml + style-guide.page-layouts.yaml -->
...
[Button usage rules]
<!-- Engine: style-guide.css-snippets.yaml (target-section: buttons) -->
...
[Input usage rules]
<!-- Engine: style-guide.css-snippets.yaml (target-section: inputs) -->
...
<!-- Engine: style-guide.shell-layouts.yaml -->
...
<!-- Engine: style-guide.semantic-classes.yaml -->
...
<!-- Engine: style-guide.css-snippets.yaml (target-section: cards) -->
...
<!-- Engine: inherits from base theme -->
...
<!-- Engine: style-guide.css-snippets.yaml -->
...
- [styles/](../styles/) — The six source style specifications

...
_load_composite_docker_services() {
  ...
  local project_name
  project_name="$(_yq_eval '.project.name // ""' "$yaml" 2>/dev/null || echo "")"
  ...
  local domains
  domains="$(_yq_eval '.project.projects[] | .domain' "$yaml" 2>/dev/null || true)"

  for domain in $domains; do
    local svc_count
    svc_count="$(_yq_eval ".project.projects[] | select(.domain==\"$domain\") | .services | length // 0" "$yaml" 2>/dev/null || echo "0")"
    [[ "$svc_count" -eq 0 || "$svc_count" == "null" ]] && continue

    local _dc_base_path
    _dc_base_path="$(_yq_eval ".project.projects[] | select(.domain==\"$domain\") | .base_path // \"\"" "$yaml" 2>/dev/null || echo "")"

    local svc_idx=0
    while [[ $svc_idx -lt $svc_count ]]; do
      local svc_base=".project.projects[] | select(.domain==\"$domain\") | .services[$svc_idx]"
      local svc_name svc_context svc_dockerfile svc_registry_path svc_auto_detect svc_single_stage

      svc_name="$(_yq_eval "${svc_base}.name" "$yaml" 2>/dev/null || echo "")"
      [[ -z "$svc_name" || "$svc_name" == "null" ]] && { svc_idx=$((svc_idx + 1)); continue; }

      svc_context="$(_yq_eval "${svc_base}.context // \"\"" "$yaml" 2>/dev/null || echo "")"
      svc_dockerfile="$(_yq_eval "${svc_base}.dockerfile // \"\"" "$yaml" 2>/dev/null || echo "")"
      svc_registry_path="$(_yq_eval "${svc_base}.registry_path // \"\"" "$yaml" 2>/dev/null || echo "")"
      svc_auto_detect="$(_yq_eval "${svc_base}.auto_detect // \"\"" "$yaml" 2>/dev/null || echo "")"
      svc_single_stage="$(_yq_eval "${svc_base}.single_stage // \"\"" "$yaml" 2>/dev/null || echo "")"
      ...
      local arg_count
      arg_count="$(_yq_eval "${svc_base}.build_args | keys | length // 0" "$yaml" 2>/dev/null || echo "0")"
      if [[ "$arg_count" -gt 0 && "$arg_count" != "null" ]]; then
        local args_str=""
        while IFS= read -r line; do
          [[ -n "$line" ]] && args_str+="${line}"$'\n'
        done < <(_yq_eval "${svc_base}.build_args | to_entries | .[] | .key + \"=\" + (.value | tostring)" "$yaml" 2>/dev/null || true)
        [[ -n "$args_str" ]] && _DOCKER_BUILD_ARGS_MAP["$image_key"]="$args_str"
      fi

      local svc_platform
      svc_platform="$(_yq_eval "${svc_base}.platform // \"\"" "$yaml" 2>/dev/null || echo "")"
      [[ -n "$svc_platform" && "$svc_platform" != "null" ]] && _DOCKER_PLATFORM_MAP["$image_key"]="$svc_platform"

      [[ "$svc_single_stage" == "true" ]] && _DOCKER_SINGLE_STAGE_MAP["$image_key"]="true"
...
_load_flat_docker_services() {
  ...
  local img_count
  img_count="$(_yq_eval '.project.docker.images | length // 0' "$yaml" 2>/dev/null || echo "0")"
  [[ "$img_count" -eq 0 || "$img_count" == "null" ]] && return

  local img_idx=0
  while [[ $img_idx -lt $img_count ]]; do
    local img_base=".project.docker.images[$img_idx]"
    local img_name img_context img_dockerfile img_registry_path img_single_stage

    img_name="$(_yq_eval "${img_base}.name" "$yaml" 2>/dev/null || echo "")"
    [[ -z "$img_name" || "$img_name" == "null" ]] && { img_idx=$((img_idx + 1)); continue; }

    img_context="$(_yq_eval "${img_base}.context // \"\"" "$yaml" 2>/dev/null || echo "")"
    img_dockerfile="$(_yq_eval "${img_base}.dockerfile // \"\"" "$yaml" 2>/dev/null || echo "")"
    img_registry_path="$(_yq_eval "${img_base}.registry_path // \"\"" "$yaml" 2>/dev/null || echo "")"
    img_single_stage="$(_yq_eval "${img_base}.single_stage // \"\"" "$yaml" 2>/dev/null || echo "")"
    ...
    local arg_count
    arg_count="$(_yq_eval "${img_base}.build_args | keys | length // 0" "$yaml" 2>/dev/null || echo "0")"
    if [[ "$arg_count" -gt 0 && "$arg_count" != "null" ]]; then
      local args_str=""
      while IFS= read -r line; do
        [[ -n "$line" ]] && args_str+="${line}"$'\n'
      done < <(_yq_eval "${img_base}.build_args | to_entries | .[] | .key + \"=\" + (.value | tostring)" "$yaml" 2>/dev/null || true)
      [[ -n "$args_str" ]] && _DOCKER_BUILD_ARGS_MAP["$img_name"]="$args_str"
    fi

    local img_platform
    img_platform="$(_yq_eval "${img_base}.platform // \"\"" "$yaml" 2>/dev/null || echo "")"
    [[ -n "$img_platform" && "$img_platform" != "null" ]] && _DOCKER_PLATFORM_MAP["$img_name"]="$img_platform"

    [[ "$img_single_stage" == "true" ]] && _DOCKER_SINGLE_STAGE_MAP["$img_name"]="true"
...
_load_yaml_docker_repos() {
  ...
  local project_type
  project_type="$(_yq_eval '.project.type // ""' "$yaml" 2>/dev/null || true)"
  [[ -z "$project_type" && "$(_yq_eval '.project // ""' "$yaml" 2>/dev/null)" == "" ]] && return

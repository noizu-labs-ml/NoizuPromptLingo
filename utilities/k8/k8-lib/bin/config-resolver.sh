...
_cfg() {
  ...
  _k8_yq_eval "$1 // \"\"" "$_K8_MERGED_CONFIG" 2>/dev/null
}
...
_dc_file_get() {
  ...
  _k8_detect_yq_mode
  if [[ "$_K8_YQ_MODE" == "mikefarah" ]]; then
    yq_cmd=(yq ea -r ". as \$item ireduce ({}; . * \$item) | ${expr} // \"\"" -)
  else
    yq_cmd=(yq -r -s "reduce .[] as \$item ({}; . * \$item) | ${expr} // \"\"" -)
  fi

defmodule StarterWeb.AuthController do
  ...
  def request_magic_link(conn, %{"email" => email}) do
    ...
    case Starter.Auth.SmartTokenAuth.request_magic_link(email) do
      {:ok, %{encoded_key: encoded_key, user: _user}} ->
        magic_link = "#{frontend_url}/auth/verify?token=#{encoded_key}"
        Starter.Auth.SmartTokenEmail.send_magic_link(email, magic_link)
  ...
  def request_otp_login(conn, %{"email" => email}) do
    case Starter.Auth.SmartTokenAuth.request_otp_login(email) do
      {:ok, %{otp_code: otp_code, user: _user}} ->
        Starter.Auth.SmartTokenEmail.send_otp_login(email, otp_code)
  ...
  def request_password_reset(conn, %{"email" => email}) do
    case Starter.Auth.SmartTokenAuth.request_password_reset(email) do
      {:ok, %{otp_code: otp_code, user: _user}} ->
        Starter.Auth.SmartTokenEmail.send_password_reset(email, otp_code)
  ...
  def send_verification(conn, _params) do
    ...
    case Starter.Auth.SmartTokenAuth.request_email_verification(user.email) do
      {:ok, %{encoded_key: encoded_key}} ->
        verification_link = "#{frontend_url}/auth/verify-email?token=#{encoded_key}"
        Starter.Auth.SmartTokenEmail.send_verification_email(user.email, verification_link)

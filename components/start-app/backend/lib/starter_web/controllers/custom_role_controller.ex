defmodule StarterWeb.CustomRoleController do
  ...
  def index(conn, %{"org_id" => org_id}) do
    ...
    case Authz.authorize(user_id, "organization", org_id, "viewer") do
      {:ok, _} ->
        roles = Starter.Repo.all(from r in RoleSchema, where: r.organization_id == ^org_id and r.is_active == true)
  ...
  def show(conn, %{"org_id" => org_id, "id" => role_id}) do
    ...
    case Authz.authorize(user_id, "organization", org_id, "viewer") do
      {:ok, _} ->
        case Starter.Repo.one(from r in RoleSchema, where: r.id == ^role_id and r.organization_id == ^org_id) do
          nil -> conn |> put_status(:not_found) |> json(%{error: "Role not found"})
          role ->
            permissions = Starter.Repo.all(from p in PermSchema, where: p.role_id == ^role_id, select: p.permission)
  ...
  def add_permission(conn, %{"org_id" => org_id, "role_id" => role_id, "permission" => permission}) do
    ...
    if Authz.check_permission(user_id, "organization", org_id, "organization:manage_settings") do
      case %PermSchema{} |> PermSchema.changeset(%{role_id: role_id, permission: permission}) |> Starter.Repo.insert() do
        {:ok, _} ->
          permissions = Starter.Repo.all(from p in PermSchema, where: p.role_id == ^role_id, select: p.permission)
  ...
  def remove_permission(conn, %{"org_id" => org_id, "role_id" => role_id, "permission_id" => perm_id}) do
    ...
    if Authz.check_permission(user_id, "organization", org_id, "organization:manage_settings") do
      case Starter.Repo.one(from p in PermSchema, where: p.id == ^perm_id and p.role_id == ^role_id) do

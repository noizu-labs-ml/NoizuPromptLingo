defmodule Starter.Users.Media do
  alias Starter.Users.Media.Asset, as: Entity
  alias Starter.Schema.Users.Media.Asset, as: Schema
  use Noizu.Repo
  ...
  def list_for_user(user_id, context, options \\ []) do
    import Ecto.Query
    settings = Noizu.Entity.Meta.persistence(Entity) |> hd
    Starter.Repo.all(from m in Schema, where: m.user_id == ^user_id)
    |> Enum.map(fn record ->
      {:ok, entity} = Entity.from_record(record, settings, context, options)
      {:ok, entity} = __after_get__(entity, context, options)
      entity

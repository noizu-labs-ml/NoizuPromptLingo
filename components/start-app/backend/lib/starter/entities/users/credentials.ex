defmodule Starter.Users.Credentials do
  @moduledoc """
  Context for Starter.Users.Credentials
  """
  alias Starter.Users.Credentials.UserCredential, as: Entity
  alias Starter.Schema.Users.Credentials.UserCredential, as: Schema
  use Noizu.Repo
  ...
  def register(user, {:login, {email, password}}, context, _options) do
    ...
    {:ok, description} =
      %Starter.Versioned.Strings.String{
        content: "",
        ...
        [credential] ->
          if Bcrypt.verify_pass(password, credential.settings["password"]) do
            with {:ok, credential_entity} <-
              ...
              |> Starter.EntityRepo.create(context, options)
            end
          else

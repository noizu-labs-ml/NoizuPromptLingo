        ...
        <li key={comment.id} className="flex gap-x-4 py-5">
          ...
          <div className="flex-auto">
            <div className="flex items-baseline justify-between gap-x-4">
              <p className="text-sm/6 font-semibold text-gray-900 dark:text-white">{comment.name}</p>
              <p className="flex-none text-xs text-gray-600 dark:text-gray-400">
                <time dateTime={comment.dateTime}>{comment.date}</time>
              </p>
            </div>
            <p className="mt-1 line-clamp-2 text-sm/6 text-gray-600 dark:text-gray-400">{comment.content}</p>
          </div>
        </li>
      ))}
    </ul>

...
This document adapts the Seven Foundations of Design framework into applied guidance for interface design. The theoretical foundations — including historical lineage, structural types (dialectical, sequential, axial), and the directed-graph topology of how foundations relate — are embedded throughout the sections below.
...
| **Nocturne** | Luminosity-based proximity — brighter elements cluster perceptually | Light-on-dark contrast creates automatic figure-ground separation | Glow halos and light falloff imply connections the eye completes |
...
| Geometric + soft edges + glow | Immersive, atmospheric | Nocturne (4-8px radius, softened by glow) |
...
- **Nocturne**: Depth through light falloff, not shadow — glow intensity implies elevation; closer elements are brighter
...
**The Japanese concept of *ma* (間)** — meaningful emptiness, the interval, the pause — versus Western *horror vacui* (fear of empty space). The six style specs sit on this spectrum:

```
← ma (void-embracing)                                    horror vacui (void-filling) →

  Nocturne     Minimal Tech    Editorial    Corporate    Consumer Playful    Bold Expressive
      ●            ●              ●            ●               ●                  ●
  (darkness    (structured    (generous     (moderate,     (dense but         (either extreme
   IS void)    emptiness)     margins)      predictable)    colorful)          void or extreme
                                                                               density)
```
...
- **Nocturne**: Luminosity-guided flow; the eye follows brightness like a path of light through darkness
...
| **Nocturne** | Connotation + Denotation | Darkness connotes expertise and immersion; data on dark canvas denotes with high fidelity |
...
- **Nocturne**: Glow-based signifiers — interactive elements emit light; hover brightens, press intensifies; the dark canvas makes any luminosity change immediately perceptible
...
- [styles/](styles/) — Six style specs demonstrating all foundations in practice

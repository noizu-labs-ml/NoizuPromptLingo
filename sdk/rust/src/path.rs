    ...
    match seg {
        ...
        Segment::Index(idx) => {
            ...
            let resolved = if *idx < 0 {
                ...
                if r < 0 {
                    return Err(anyhow!(
                        "negative index {} out of range for len {}",
                        idx,
                        seq.len()
                    ));
                }
        ...
        Segment::Wildcard => Err(anyhow!("wildcard [*] is not supported in set_path")),
        Segment::Length => Err(anyhow!(".length is not supported in set_path")),
    }
}

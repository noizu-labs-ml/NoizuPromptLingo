defmodule NoizuPromptLingua.Users.Credentials do
  @moduledoc """
  Context for NoizuPromptLingua.Users.Credentials
  """
  alias NoizuPromptLingua.Users.Credentials.UserCredential, as: Entity
  alias NoizuPromptLingua.Schema.Users.Credentials.UserCredential, as: Schema
  use Noizu.Repo
  import Ecto.Query, only: [from: 2]

  def_repo(entity: NoizuPromptLingua.Users.Credentials.UserCredential)
  ...
  def register(user, {:login, {email, password}}, context, _options) do
    ...
    {:ok, description} =
      %NoizuPromptLingua.Versioned.Strings.String{
        content: "",
  ...
  def authenticate({:login, {email, password}}, context, options) do
    ...
    with :valid <- valid_login?(email, password) do
      ...
      case NoizuPromptLingua.Repo.all(q) do
        [] ->
        ...
        [credential] ->
          if Bcrypt.verify_pass(password, credential.settings["password"]) do
            with {:ok, credential_entity} <-
                   NoizuPromptLingua.Users.Credentials.UserCredential.entity(credential.id, context),
              ...
              |> NoizuPromptLingua.EntityRepo.create(context, options)
            end
          else

defmodule NoizuPromptLingua.Authz.Policies do
  alias NoizuPromptLingua.Authz.Policies.Policy, as: Entity
  alias NoizuPromptLingua.Schema.Authz.Policy, as: Schema
  alias NoizuPromptLingua.Schema.Authz.UserPolicy, as: UserPolicySchema
  ...
  def list_active(opts \\ []) do
    ...
    NoizuPromptLingua.Repo.all(query)
  end

  def get_by_name(name) do
    NoizuPromptLingua.Repo.one(from p in Schema, where: p.name == ^name and p.is_active == true)
  end
    ...
    case NoizuPromptLingua.Repo.get(Schema, id) do
      ...
      policy ->
        if policy.is_system do
          ...
          policy |> Schema.changeset(attrs) |> NoizuPromptLingua.Repo.update()
        end
    end
  end
    ...
    case NoizuPromptLingua.Repo.get(Schema, id) do
      ...
      policy ->
        if policy.is_system do
          ...
          NoizuPromptLingua.Repo.delete(policy)
        end
    end
  end

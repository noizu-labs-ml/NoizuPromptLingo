defmodule NoizuPromptLingua.Domains.Tickets.Tools.TicketLinkEntity do
  use Noizu.MCP.Server.Tool,
    ...
    description: "Link a ticket to a non-ticket entity (customer_persona, customer_segment, campaign, keyword, competitor, market_report, landing_page, domain_name, ad_group, ad_copy).",
    hidden: true,
    category: "Tickets"
  ...
  def call(args, _ctx) do
    ...
    case Links.link_entity(ticket_id, entity_type, entity_id, opts) do
      {:ok, link} ->
        {:ok, %{id: link.id, ticket_id: ticket_id, entity_type: link.entity_type, entity_id: entity_id, link_type: link.link_type}}

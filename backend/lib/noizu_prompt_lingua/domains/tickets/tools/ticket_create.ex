defmodule NoizuPromptLingua.Domains.Tickets.Tools.TicketCreate do
  ...
  input_schema %{
    ...
    "properties" => %{
      ...
      "project" => %{"type" => "string", "description" => "Optional project slug or UUID to scope this ticket to"},
  ...
  def call(args, _ctx) do
    ...
    with {:org, org_id} when not is_nil(org_id) <- {:org, Resolve.organization_id(org_ref)},
      ...
      attrs =
        extract(args, ~w(title description ticket_type status priority assignee reporter queue_id parent_id custom_fields))
      ...
      case Tickets.create(attrs) do
        {:ok, ticket} ->
          {:ok, %{
            ...
            organization_id: ticket.organization_id, project_id: ticket.project_id,

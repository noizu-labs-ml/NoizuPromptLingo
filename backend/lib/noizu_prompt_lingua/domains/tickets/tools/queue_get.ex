  ...
  def call(args, _ctx) do
    ...
    with {:ok, org_id, project_id} <- Resolve.scope(Args.get(args, :organization), Args.get(args, :project)),
      ...
      {:ok, %{
        ...
        stages: Enum.map(board.stages, &%{id: &1.id, slug: &1.slug, name: &1.name, kind: &1.kind, position: &1.position, wip_limit: &1.wip_limit}),
        iterations: Enum.map(board.iterations, &%{id: &1.id, name: &1.name, sequence: &1.sequence, status: &1.status, goal: &1.goal, starts_on: &1.starts_on, ends_on: &1.ends_on})
      }}
    else

defmodule NoizuPromptLingua.Domains.Tickets.Tools.DefinitionCreate do
  ...
  input_schema(%{
    ...
    "properties" => %{
      "organization" => %{
        "type" => "string",
        "description" => "Organization slug or UUID. Omit for a global type."
      },
      "project" => %{
        "type" => "string",
        "description" =>
          "Project slug or UUID. Requires organization; scopes the type to a project."
      },
      "slug" => %{
        "type" => "string",
        "description" => "Slug, unique within the chosen scope (e.g. \"bug\", \"epic\")"
      },
      ...
      "status_workflow" => %{
        "type" => "object",
        "description" => "{statuses: [...], transitions: {status: [targets]}}"
      },
      "fields" => %{
        "type" => "array",
        "description" =>
          "Array of {slug, required} field assignments (slugs resolved within scope)",
        "items" => %{
          "type" => "object",
          "properties" => %{
            "slug" => %{"type" => "string"},

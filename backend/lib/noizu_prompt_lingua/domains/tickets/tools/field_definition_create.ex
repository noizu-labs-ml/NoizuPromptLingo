defmodule NoizuPromptLingua.Domains.Tickets.Tools.FieldDefinitionCreate do
  ...
  input_schema(%{
    ...
    "properties" => %{
      "organization" => %{
        "type" => "string",
        "description" => "Organization slug or UUID. Omit for a global definition."
      },
      "project" => %{
        "type" => "string",
        "description" =>
          "Project slug or UUID. Requires organization; scopes the field to a project."
      },
      "slug" => %{
        "type" => "string",
        "description" => "Slug, unique within the chosen scope (e.g. \"story_points\")"
      },
      "label" => %{"type" => "string", "description" => "Display label"},
      "field_type" => %{
        "type" => "string",
        "description" =>
          "One of: text, rich_text, markdown, radio, select, multi_select, number, date, persona, url"
      },
      "options" => %{
        "type" => "object",
        "description" => "For select/radio/multi_select: {values: [{value, label}]}"
      },
          ...
          {:ok, field} ->
            {:ok,
             %{
               ...
               scope: Atom.to_string(Definitions.scope_of(field))
             }}

defmodule NoizuPromptLingua.Domains.Tickets.Tools.FieldDefinitionUpdate do
  ...
  input_schema %{
    "type" => "object",
    "properties" => %{
      "organization" => %{"type" => "string", "description" => "Organization slug or UUID. Omit to target a global definition."},
      "project" => %{"type" => "string", "description" => "Project slug or UUID — targets the project-scoped definition."},
      "slug" => %{"type" => "string", "description" => "Field slug to update (within the chosen scope)"},
      ...
      "disabled" => %{"type" => "boolean", "description" => "Set true to suppress (tombstone) the inherited definition"}
    },
    "required" => ["slug"]
  }
  ...
  def call(args, _ctx) do
    ...
    with {:ok, org_id, project_id} <- Resolve.scope(Args.get(args, :organization), Args.get(args, :project)),
      ...
      case Definitions.update_field(id, attrs) do
        {:ok, field} -> {:ok, %{id: field.id, slug: field.slug, label: field.label, field_type: field.field_type}}

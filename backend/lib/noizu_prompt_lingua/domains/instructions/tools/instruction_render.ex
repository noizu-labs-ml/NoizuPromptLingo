defmodule NoizuPromptLingua.Domains.Instructions.Tools.InstructionRender do
  use Noizu.MCP.Server.Tool, name: "Instruction.Render",
    description: "Render an instruction's body with task params — the prompt to hand a sub-agent. Reference by slug handle so the full text never has to be re-sent.",
  ...
  input_schema %{
    ...
    "properties" => %{
      ...
      "params" => %{"type" => "object", "description" => "Param name → value, substituted into {{name}} placeholders"},
      "version" => %{"type" => "integer", "description" => "Specific version (defaults to active)"}
    },
  ...
  def call(args, _ctx) do
    ...
    case Instructions.render(instruction, params, opts) do
      ...
      {:error, :not_found} -> {:error, "Instruction '#{key}' not found"}
      {:error, :version_not_found} -> {:error, "Version not found"}
      {:error, {:missing_params, missing}} -> {:error, "Missing required params: #{Enum.join(missing, ", ")}"}
    end
  end
end

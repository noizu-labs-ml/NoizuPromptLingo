defmodule NoizuPromptLingua.Domains.Customers.Tools.PersonaCreate do
  use Noizu.MCP.Server.Tool,
    ...
    description: "Create a customer/user persona (ICP) — the marketing audience model, distinct from agent personas.",
    hidden: true,
    category: "Customers"
  ...
  input_schema %{
    ...
    "properties" => %{
      ...
      "project" => %{"type" => "string", "description" => "Optional project slug or UUID to scope this persona"},
      ...
      "demographics" => %{"type" => "object", "description" => "Demographics (age/role/income/location/company size)"},
      "goals" => %{"type" => "array", "items" => %{"type" => "string"}, "description" => "Goals"},
      "pains" => %{"type" => "array", "items" => %{"type" => "string"}, "description" => "Pain points"},
      "channels" => %{"type" => "array", "items" => %{"type" => "string"}, "description" => "Channels to reach this persona"},
  ...
  def call(args, _ctx) do
    ...
    with {:org, org_id} when not is_nil(org_id) <- {:org, Resolve.organization_id(org_ref)},
      ...
      case Customers.create_persona(attrs) do
        {:ok, p} ->
          {:ok, %{id: p.id, slug: p.slug, name: p.name, organization_id: p.organization_id, project_id: p.project_id, status: p.status}}
        {:error, changeset} ->

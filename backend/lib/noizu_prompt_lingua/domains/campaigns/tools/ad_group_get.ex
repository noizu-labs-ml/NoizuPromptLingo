defmodule NoizuPromptLingua.Domains.Campaigns.Tools.AdGroupGet do
  ...
  def call(args, _ctx) do
    case Campaigns.get_ad_group(Args.get(args, :id)) do
      nil -> {:error, "Ad group not found"}
      g -> {:ok, %{id: g.id, slug: g.slug, name: g.name, theme: g.theme, keywords: g.keywords, bid_cents: g.bid_cents, status: g.status, campaign_id: g.campaign_id}}
    end
  end
end

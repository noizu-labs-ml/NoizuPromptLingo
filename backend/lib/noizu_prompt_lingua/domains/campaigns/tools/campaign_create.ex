defmodule NoizuPromptLingua.Domains.Campaigns.Tools.CampaignCreate do
  ...
  input_schema(%{
    ...
    "properties" => %{
      ...
      "status" => %{
        "type" => "string",
        "description" => "draft (default), active, paused, completed, archived"
      },
      ...
      "segment_id" => %{
        "type" => "string",
        "description" => "Optional target customer_segment UUID"
      },
      "targeting" => %{
        "type" => "object",
        "description" => "Targeting config (audiences/geo/keywords)"
      },
  ...
  def call(args, _ctx) do
    ...
    with {:org, org_id} when not is_nil(org_id) <- {:org, Resolve.organization_id(org_ref)},
      ...
      case Campaigns.create_campaign(attrs) do
        {:ok, c} ->
          {:ok, %{id: c.id, slug: c.slug, name: c.name, channel: c.channel, status: c.status}}

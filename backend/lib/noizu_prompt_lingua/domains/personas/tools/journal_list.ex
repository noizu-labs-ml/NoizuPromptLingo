defmodule NoizuPromptLingua.Domains.Personas.Tools.JournalList do
  use Noizu.MCP.Server.Tool, name: "Persona.Journal.List",
    description: "List a persona's work-log/journal entries (most recent first).",
    hidden: true, category: "Personas.Journal", annotations: [read_only_hint: true]
  ...
  def call(args, _ctx) do
    ...
    case persona do
      ...
      p ->
        ...
        {:ok, %{count: length(entries), entries: Enum.map(entries, fn j ->
          %{id: j.id, category: j.category, title: j.title, body: j.body,
            actor: j.actor, tags: j.tags, at: j.inserted_at}
        end)}}
    end
  end
end

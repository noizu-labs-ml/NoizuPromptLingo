defmodule NoizuPromptLingua.Domains.Assets.Tools.AssetGenerate do
  use Noizu.MCP.Server.Tool,
    name: "Asset.Generate",
    description:
      "Generate content from prompt YAML using LLM with format-specific references. Creates artifact with generated content.",
    hidden: true,
    category: "Assets"
  ...
  input do
    ...
    field :llm_generate, :boolean,
      description: "Call LLM to generate content (default true). Set false for placeholder only."
  ...
  def call(args, _ctx) do
    ...
    llm_generate =
      case args[:llm_generate] || args["llm_generate"] do
        false -> false
        "false" -> false
        _ -> true
      end

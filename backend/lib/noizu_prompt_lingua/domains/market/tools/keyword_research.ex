defmodule NoizuPromptLingua.Domains.Market.Tools.KeywordResearch do
  use Noizu.MCP.Server.Tool,
    ...
    description: "LLM-generate a set of candidate keywords (term/intent/volume/difficulty/cpc) for a topic and bulk-insert them.",
    hidden: true,
    category: "Market.Keywords"

  input do
    field :organization, :string, required: true, description: "Organization slug or UUID"
    field :topic, :string, required: true, description: "Topic / seed phrase to research keywords for"
  ...
  def call(args, _ctx) do
    ...
    with {:org, org_id} when not is_nil(org_id) <- {:org, Resolve.organization_id(org_ref)},
      ...
      case Market.research_keywords(org_id, project_id, Args.get(args, :topic), opts) do
        {:ok, keywords} ->
          {:ok, %{created: length(keywords), keywords: Enum.map(keywords, &%{id: &1.id, term: &1.term, intent: &1.intent, volume: &1.volume, difficulty: &1.difficulty})}}
        {:error, reason} ->

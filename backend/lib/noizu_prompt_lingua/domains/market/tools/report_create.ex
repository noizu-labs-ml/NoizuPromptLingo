defmodule NoizuPromptLingua.Domains.Market.Tools.ReportCreate do
  use Noizu.MCP.Server.Tool,
    ...
    description:
      "Create a market/competitor analysis report shell (body is generated separately via MarketReport.Generate).",
    hidden: true,
  ...
  input_schema(%{
    ...
    "properties" => %{
      ...
      "report_type" => %{
        "type" => "string",
        "description" => "market_analysis (default), competitor_analysis, swot, keyword_summary"
      },
      "summary" => %{"type" => "string", "description" => "Short abstract"},
      "competitor_id" => %{
        "type" => "string",
        "description" => "Optional competitor UUID for competitor-scoped reports"
      },
  ...
  def call(args, _ctx) do
    ...
    with {:org, org_id} when not is_nil(org_id) <- {:org, Resolve.organization_id(org_ref)},
      ...
      case Market.create_report(attrs) do
        {:ok, r} ->
          {:ok,
           %{id: r.id, slug: r.slug, title: r.title, report_type: r.report_type, status: r.status}}

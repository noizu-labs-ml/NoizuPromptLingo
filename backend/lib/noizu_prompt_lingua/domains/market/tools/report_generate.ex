defmodule NoizuPromptLingua.Domains.Market.Tools.ReportGenerate do
  use Noizu.MCP.Server.Tool,
    ...
    description: "Generate a market report body via LLM, store it as an artifact, and set the report's summary + artifact_id (status → ready).",
    hidden: true,
    category: "Market.Reports"

  input do
    field :id, :string, required: true, description: "Report UUID"
    field :prompt, :string, description: "Optional custom generation prompt (defaults to a prompt built from the report type/title)"
  ...
  def call(args, _ctx) do
    ...
    case Market.generate_report(id, opts) do
      {:ok, r} -> {:ok, %{id: r.id, slug: r.slug, artifact_id: r.artifact_id, status: r.status, llm_generated: llm_generate}}
      {:error, :not_found} -> {:error, "Market report '#{id}' not found"}

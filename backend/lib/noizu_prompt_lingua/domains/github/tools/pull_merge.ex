defmodule NoizuPromptLingua.Domains.Github.Tools.PullMerge do
	...
	input do
		...
		field :merge_method, :string, description: "Merge method (merge|squash|rebase)", required: false
	end
	...
	def call(args, _ctx) do
		...
		body = Map.take([commit_title: Args.get(args, :commit_title), merge_method: Args.get(args, :merge_method)], [:commit_title, :merge_method])
			|> Enum.filter(fn {_, v} -> v != nil end)
			|> Enum.into(%{})

		case {parse_uuid(caller_user_id), ensure_org(org_id)} do
			{{:ok, user_uuid}, {:ok, org}} -> Client.merge_pull(user_uuid, org, repo_ref, pull_number, body)

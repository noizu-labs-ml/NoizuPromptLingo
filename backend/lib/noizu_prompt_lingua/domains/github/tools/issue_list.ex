defmodule NoizuPromptLingua.Domains.Github.Tools.IssueList do
  use Noizu.MCP.Server.Tool,
    name: "Github.IssueList",
  ...
  def call(args, _ctx) do
    ...
    opts =
      Keyword.take(
        [
          state: Args.get(args, :state),
          page: Args.get(args, :page),
          per_page: Args.get(args, :per_page)
        ],
        [:state, :page, :per_page]
      )
    ...
    case {parse_uuid(caller_user_id), ensure_org(org_id)} do
      {{:ok, user_uuid}, {:ok, org}} -> Client.list_issues(user_uuid, org, repo_ref, opts)

defmodule NoizuPromptLingua.Domains.Github.Tools.BranchList do
	use Noizu.MCP.Server.Tool,
		name: "Github.BranchList",
		description: "List branches for a repository. Requires read access.",
		hidden: true,
		category: "GitHub",
	...
	def call(args, _ctx) do
		...
		opts = [page: Args.get(args, :page), per_page: Args.get(args, :per_page)]
			|> Enum.filter(fn {_, v} -> v != nil end)

		case {parse_uuid(caller_user_id), ensure_org(org_id)} do
			{{:ok, user_uuid}, {:ok, org}} -> Client.list_branches(user_uuid, org, repo_ref, opts)

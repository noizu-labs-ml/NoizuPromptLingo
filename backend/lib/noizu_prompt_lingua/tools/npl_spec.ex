defmodule NoizuPromptLingua.Tools.NPLSpec do
  ...
  use Noizu.MCP.Server.Tool,
    name: "NPLSpec",
    description:
      "Noizu Prompt Lingua specification generation and formatting. Generates a versioned " <>
        "NPL specification block from the convention library. Use components/rendered to select " <>
        "a subset; empty components includes all conventions.",
    category: "NPL",
  ...
  input do
    field :components, {:array, :object},
      ...
      field :component_priority, :integer,
        default: 0,
        description: "Maximum component priority to include"

      field :example_priority, :integer,
        default: 0,
        description: "Maximum example priority to include"
    end
  ...
  def call(args, _ctx) do
    components = normalize_specs(NoizuPromptLingua.MCP.Args.get(args, :components))
    rendered = normalize_specs(NoizuPromptLingua.MCP.Args.get(args, :rendered))
    component_priority = NoizuPromptLingua.MCP.Args.get(args, :component_priority) || 0
    example_priority = NoizuPromptLingua.MCP.Args.get(args, :example_priority) || 0
    extension = NoizuPromptLingua.MCP.Args.get(args, :extension) || false
    concise = NoizuPromptLingua.MCP.Args.get(args, :concise) || true
    xml = NoizuPromptLingua.MCP.Args.get(args, :xml) || false

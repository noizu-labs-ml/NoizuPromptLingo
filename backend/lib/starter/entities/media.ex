defmodule Starter.Media do
  alias Starter.Media.Asset, as: Entity
  alias Starter.Schema.Media.Asset, as: Schema
  use Noizu.Repo
  ...
  def get_cached_variant(media_id, canonical_params) do
    import Ecto.Query
    Starter.Repo.one(
      from v in Starter.Schema.Media.Variant,
      ...
      nil ->
        with {:ok, original_binary} <- fetch_from_s3(media.file),
             {:ok, transformed, content_type} <-
               Starter.Media.Transform.transform(original_binary, transform_params) do
          variant_key = Starter.Media.Transform.variant_s3_key(media.file, transform_params)

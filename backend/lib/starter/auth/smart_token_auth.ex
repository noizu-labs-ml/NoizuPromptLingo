defmodule Starter.Auth.SmartTokenAuth do
    ...
    case find_active_user_by_email(email) do
      {:ok, user} ->
        ...
        case find_and_authorize_otp(user_ref, otp_code, {:auth, :password_reset}, conn, context) do
          {:ok, _token} ->
            Starter.Users.Credentials.update_password(user, new_password, context)
    ...
    case SmartToken.authorize!(token_key, %{}, context) do
      {:ok, token} ->
        with {:ok, user} <- Noizu.EntityReference.Protocol.entity(token.resource, context) do
          ...
          |> Starter.Repo.update_all(set: [verified: true])
  ...
  defp find_active_user_by_email(email) do
    ...
    case Starter.Repo.one(q) do
      ...
      record ->
        settings = Noizu.Entity.Meta.persistence(Starter.Users.User) |> hd
        Starter.Users.User.from_record(record, settings, Noizu.Context.system(), [])
    end
  end
end

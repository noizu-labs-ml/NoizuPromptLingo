defmodule NoizuPromptLinguaWeb.SSOController do
  ...
  def exchange(conn, %{"code" => code}) do
    with {:ok, session_id} <- NoizuPromptLingua.Auth.SSOCode.exchange(code),
         {:ok, session} <- NoizuPromptLingua.Users.Sessions.get(session_id, Noizu.Context.system()),
  ...
  defp handle_sso_callback(conn, provider_type, attrs) do
    ...
    case NoizuPromptLingua.Auth.SSO.authenticate_sso(provider_type, attrs) do
      {:ok, session} ->
        {:ok, code} = NoizuPromptLingua.Auth.SSOCode.create(session.id)

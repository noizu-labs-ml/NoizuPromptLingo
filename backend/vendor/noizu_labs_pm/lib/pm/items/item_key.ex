defmodule Noizu.PM.Items.ItemKey do
  @moduledoc """
  Canonical human-readable item key generation. A key is
  ...
  `Noizu.PM.Items` domain (they need Repo). This module owns the two deterministic
  ...
  #{@prefix_max} (e.g. "noizu-infra" -> "NOIZUI"). Falls back to "ITM" when a slug
  ...
  def derive_prefix(slug) do
    ...
    # >= 2 chars so it always satisfies the key_prefix format (single-char/empty
    # slugs fall back rather than produce an invalid 1-char prefix).
    if String.length(base) < 2, do: "ITM", else: base
  end

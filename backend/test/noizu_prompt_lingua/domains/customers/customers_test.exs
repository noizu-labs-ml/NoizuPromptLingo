defmodule NoizuPromptLingua.Domains.CustomersTest do
  ...
  test "create + resolve a customer persona by slug", %{org_id: org_id} do
    {:ok, p} =
      Customers.create_persona(%{
        organization_id: org_id, slug: "icp-buyer", name: "Technical Buyer",
        archetype: "Engineer", goals: ["ship faster"], pains: ["toil"]
      })
  ...
  test "segment + persona scoping and listing", %{org_id: org_id} do
    ...
    {:ok, _} = Customers.create_persona(%{organization_id: org_id, slug: "p1", name: "P1", segment_id: seg.id})
  ...
  describe "ticket linking (requirement #1)" do
    test "link a persona to a ticket and reverse-lookup", %{org_id: org_id} do
      {:ok, persona} = Customers.create_persona(%{organization_id: org_id, slug: "icp", name: "ICP"})
      {:ok, ticket} = Tickets.create(%{organization_id: org_id, title: "Build feature X", ticket_type: "task"})
      ...
      assert [%{entity_id: eid, entity_type: "customer_persona"}] = Links.get_entity_links(ticket.id)
      assert eid == persona.id
    end

    test "linking to a missing ticket is a clean error", %{org_id: org_id} do
      {:ok, persona} = Customers.create_persona(%{organization_id: org_id, slug: "icp2", name: "ICP2"})
    ...
    test "duplicate link is rejected by the unique index", %{org_id: org_id} do
      {:ok, persona} = Customers.create_persona(%{organization_id: org_id, slug: "icp3", name: "ICP3"})
  ...
  describe "draft_persona (offline path)" do
    test "llm_generate: false stores an artifact and sets summary/artifact_id", %{org_id: org_id} do
      {:ok, persona} = Customers.create_persona(%{organization_id: org_id, slug: "draftme", name: "Draft Me", goals: ["win"], pains: ["lose"]})

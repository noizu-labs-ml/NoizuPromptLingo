defmodule NoizuPromptLingua.Domains.MarketTest do
  ...
  test "keyword create with metrics + intent filter + ordering by volume", %{org_id: org_id} do
    ...
    assert [%{term: "ci tools"}] =
             Market.list_keywords(organization_id: org_id, intent: "commercial")
  ...
  describe "report generation (offline path)" do
    test "generate_report with llm_generate: false stores artifact + sets status ready", %{
      org_id: org_id
    } do
      {:ok, report} =
    ...
    test "research_keywords with llm_generate: false parses an echoed JSON array", %{
      org_id: org_id
    } do

defmodule NoizuPromptLinguaWeb.ChatMessageControllerTest do
  ...
  setup %{conn: conn} do
    ...
    created =
      post(auth_conn, "/api/v1/organizations", %{organization: %{slug: slug, name: "Msg Org"}})
    ...
    {:ok, room} =
      Chat.create_room(%{
        ...
        name: "Messages #{System.unique_integer([:positive])}"
      })
  ...
  describe "POST message — happy path" do
    test "creates a message and defaults sender to the actor", %{
      conn: conn,
      base: base,
      user: user
    } do
  ...
  describe "POST message — input hardening (Sofia G2)" do
    ...
    test "over-length content (>10k) -> 422", %{conn: conn, base: base} do
      assert json_response(
               post(conn, base, %{message: %{content: String.duplicate("a", 10_001)}}),
               422
             )["errors"]["content"]
    end
    ...
    test "content with markup is stored verbatim (XSS is a render-layer concern)", %{
      conn: conn,
      base: base
    } do
  ...
  describe "GET messages" do
    test "returns messages oldest-first with embedded reaction summaries", %{
      conn: conn,
      base: base
    } do
  ...
  describe "threaded replies (ffa2d2f6)" do
    ...
    test "POST a message with parent_message_id creates a reply", %{
      conn: conn,
      base: base,
      parent_id: pid
    } do
      reply =
        json_response(post(conn, base, %{message: %{content: "re", parent_message_id: pid}}), 201)[
          "message"
        ]
    ...
    test "GET .../messages/:id/replies returns replies chronologically with reactions", %{
      conn: conn,
      base: base,
      parent_id: pid
    } do
    ...
    test "channel list is top-level only + embeds reply_count/last_reply_at; ?include_replies=true is flat",
         %{conn: conn, base: base, parent_id: pid} do
    ...
    test "reply-to-a-reply is rejected (one-level threads) -> 422", %{
      conn: conn,
      base: base,
      parent_id: pid
    } do
      reply =
        json_response(post(conn, base, %{message: %{content: "r", parent_message_id: pid}}), 201)[
          "message"
        ]

      resp =
        json_response(
          post(conn, base, %{message: %{content: "deep", parent_message_id: reply["id"]}}),
          422
        )
    ...
    test "parent in another room is rejected -> 422", %{conn: conn, base: base, org_id: org_id} do
      ...
      {:ok, foreign} =
        Chat.send_message(%{room_id: other_room.id, content: "elsewhere", sender: "x"})

      resp =
        json_response(
          post(conn, base, %{message: %{content: "r", parent_message_id: foreign.id}}),
          422
        )
    ...
    test "unknown parent -> 422", %{conn: conn, base: base} do
      assert json_response(
               post(conn, base, %{
                 message: %{content: "r", parent_message_id: Ecto.UUID.generate()}
               }),
               422
             )["error"]
    end

    test "deleting the parent DETACHES replies to root (ON DELETE SET NULL), never deletes them",
         %{conn: conn, base: base, parent_id: pid} do
      reply =
        json_response(
          post(conn, base, %{message: %{content: "survivor", parent_message_id: pid}}),
          201
        )["message"]

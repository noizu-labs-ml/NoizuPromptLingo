defmodule NoizuPromptLinguaWeb.TicketKeyControllerTest do
  ...
  setup %{conn: conn} do
    ...
    created = post(auth, "/api/v1/organizations", %{organization: %{slug: slug, name: "TKey Org"}})
    org_id = json_response(created, 201)["organization"]["id"]

    {:ok, conn: auth, org_id: org_id, base: "/api/v1/organizations/#{org_id}/tickets"}
  end

  test "create echoes a human key + number; GET resolves by the key and by UUID", %{conn: conn, base: base} do
    t = json_response(post(conn, base, %{ticket: %{title: "Bug", ticket_type: "task"}}), 201)["ticket"]
    assert t["number"] == 1
  ...
  test "a second ticket increments the number", %{conn: conn, base: base} do
    a = json_response(post(conn, base, %{ticket: %{title: "A", ticket_type: "task"}}), 201)["ticket"]
    b = json_response(post(conn, base, %{ticket: %{title: "B", ticket_type: "task"}}), 201)["ticket"]

defmodule NoizuPromptLingua.Domains.Tickets.Seed do
  ...
  defp seed_types do
    ...
    |> Enum.each(fn {type_attrs, field_specs} ->
      {:ok, _} = Definitions.upsert_type(type_attrs)

      field_specs
      |> Enum.with_index()
      |> Enum.each(fn {{slug, required}, idx} ->
        Definitions.add_field_to_type(type_attrs[:slug], slug, required: required, position: idx)
      end)
    end)
  end
  ...
  defp types do
    [
      ...
      {%{slug: "user_story", name: "User Story",
      ...
      {%{slug: "research", name: "R&D",
         ...
         status_workflow: workflow(
           ...
           %{"open" => ["in_progress", "closed"],
             ...
             "done" => ["closed", "open"]})},
       [{"priority", true}, {"research_question", true}, {"hypothesis", false},
        {"findings", false}, {"labels", false}]},

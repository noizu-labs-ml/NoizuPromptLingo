...
## ⚠️ MANDATORY: Feature Implementation Workflow

**THIS MUST BE FOLLOWED FOR ALL FEATURES**

❌ **DO NOT** write code directly
❌ **DO NOT** write PRDs manually
❌ **DO NOT** skip test creation
✅ **ALWAYS** use the agent orchestration system
✅ **ALWAYS** start with PRD generation
✅ **ALWAYS** generate tests before implementation

### When to Use This Workflow

| Scenario | Use TDD Agents? |
|----------|-----------------|
| Adding new feature | ✅ YES - Required |
| Bug fix with test case | ✅ YES - Required |
| Refactoring existing code | ✅ YES - Create PRD first |
| Documentation update | ❌ No - Direct edit OK |
| Configuration change | ❌ No - Direct edit OK |
| Emergency hotfix | ⚠️ Use agents, fast-track if needed |

### Anti-Patterns (What NOT to Do)

#### ❌ Pattern 1: Code-First Implementation
```markdown
User: "Add feature X"
Claude: *writes code directly without PRD*
Result: No test coverage, unclear requirements, maintenance nightmare
```

**✅ Instead:**
```markdown
User: "Add feature X"
Claude Phase 1: Use npl-idea-to-spec → Creates personas + user stories
Claude Phase 2: Use npl-prd-editor → Creates detailed PRD
Claude Phase 3: Use npl-tdd-tester → Creates test suite
Claude Phase 4: Use npl-tdd-coder → Implements code
Claude Phase 5: Use npl-tdd-debugger if tests fail
Result: Full test coverage, clear requirements, maintainable code
```

#### ❌ Pattern 2: Writing PRDs Manually
```bash
❌ AVOID: Manually editing project-management/PRDs/PRD-XXX.md
```

**✅ INSTEAD:**
```bash
✅ USE: npl-prd-editor agent to generate PRD from user stories
```

#### ❌ Pattern 3: Creating Tests After Code
```bash
❌ Code written first, tests added later = incomplete coverage
```

**✅ INSTEAD:**
```bash
✅ Tests generated by npl-tdd-tester BEFORE implementation
✅ Code written to pass existing tests
```
...
### Workflow Steps (Sequential & Required)

#### Phase 1️⃣: Specification Discovery
**Agent**: `npl-idea-to-spec`
**Input**: Feature description/user request
**Output**:
- `project-management/personas/` - User personas
- `project-management/user-stories/index.yaml` - User stories

**Command**:
```bash
# Task will invoke: npl-idea-to-spec
# Provide: Feature description and context
```

#### Phase 2️⃣: PRD Generation
**Agent**: `npl-prd-editor`
**Input**: User stories from Phase 1
**Output**:
- `project-management/PRDs/PRD-XXX.md` - Complete PRD with acceptance criteria

**Command**:
```bash
# Task will invoke: npl-prd-editor
# Pass result from Phase 1
```
...
#### Phase 3️⃣: Test Suite Generation
**Agent**: `npl-tdd-tester`
**Input**: PRD from Phase 2
**Output**:
- `tests/test_feature_name.py` - Comprehensive test suite
- Test plan document

**Command**:
```bash
# Task will invoke: npl-tdd-tester
# Pass: PRD from Phase 2
# Generates: Complete test suite with fixtures, parametrization, coverage targets
```
...
#### Phase 4️⃣: Code Implementation
**Agent**: `npl-tdd-coder`
**Input**: Test suite from Phase 3
**Output**:
- Implementation code in `src/` matching test requirements

**Command**:
```bash
# Task will invoke: npl-tdd-coder
# Pass: Test suite from Phase 3
# Agent uses: mise run test-status (quick check)
#             mise run test-errors (detailed failures)
```

**Requirements**:
- All tests MUST pass before completion
- Coverage MUST meet 80% minimum
- Run `mise run test-coverage` to verify

#### Phase 5️⃣: Debug Loop (if needed)
**Agent**: `npl-tdd-debugger`
**Input**: Test failures or blockers
**Output**:
- Root cause analysis
- Routes back to appropriate agent for fixes

**Command**:
```bash
# Automatic if Phase 4 encounters issues
# Can route to: npl-prd-editor, npl-tdd-tester, or npl-tdd-coder
```

### Key Commands for TDD Agents

```bash
# ⚡ CRITICAL COMMANDS - Use These
mise run test-status        # 🟢 Quick pass/fail check (3 emoji: ✅ ⚠️ ❌)
mise run test-errors        # 📊 Detailed failure output for debugging
mise run test-coverage      # 📈 Coverage report (must be >= 80%)
mise run test-html          # 🌐 HTML coverage visualization

# Optional but helpful
mise run test               # Run all tests with verbose output
mise run test -- --lf       # Run only last failed tests
...
- ✅ **Test coverage report** before marking complete
...
# View HTML report (shows gaps)
mise run test-html
open htmlcov/index.html
```

### Artifact Checklist (Before Committing)

- [ ] **Phase 1 Complete**: Personas + user stories in `project-management/`
- [ ] **Phase 2 Complete**: PRD in `project-management/PRDs/PRD-XXX.md`
- [ ] **Phase 3 Complete**: Tests in `tests/test_*.py`
- [ ] **Phase 4 Complete**: Code in `src/`
- [ ] **All Tests Pass**: `mise run test-status` shows ✅
- [ ] **Coverage >= 80%**: `mise run test-coverage` confirms
- [ ] **No Debug Phase Needed**: Code works on first try (or debugger resolved)
...
- Phase 1 + Phase 2 (spec work): Sequential
- Phase 2 + Phase 3 (PRD → Tests): Sequential
- Phase 3 + Phase 4 (Tests → Code): Sequential
- Phase 1 can run multiple agents in parallel: `npl-idea-to-spec` (persona + story generation)

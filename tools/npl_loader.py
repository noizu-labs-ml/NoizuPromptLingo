...
def mark_deleted_sections_not_in(conn, valid_section_ids: List[str]):
    """Mark sections as deleted if not in valid_section_ids."""
    now = datetime.utcnow()
    with conn.cursor() as cur:
...
def populate_convention_definitions(conn, npl_dir: Path):
    ...
    mark_deleted_sections_not_in(conn, valid_section_ids)
...
def refresh_convention_definitions(conn, npl_dir: Path):
    ...
    # Collect all valid section IDs from all data (not just changed files)
    valid_section_ids = []
    for key, file_data in data.items():
        content = file_data["content"]
        section_name = content.get("name", key)
...
def format_component(component: dict, level: int = 3, with_labels: bool = False) -> List[str]:
    ...
    # Level 0 examples only
    examples = component.get("examples", [])
    level0_examples = [e for e in examples if e.get("level", 0) == 0]
    if level0_examples:
        lines.append("**Example**:")
        lines.append("")
        for ex in level0_examples:
...
def format_section(name: str, data: dict, level: int = 2, with_labels: bool = False) -> List[str]:
    ...
    if components:
        for component in components:
            lines.extend(format_component(component, level + 1, with_labels))
...
def format_npl_output(data: Dict[str, Dict], with_labels: bool = False) -> str:
    ...
    for section_key in section_order:
        if section_key in data:
            lines.extend(format_section(section_key, data[section_key], with_labels=with_labels))
            processed.add(section_key)

    # Process remaining sections
    for key, section_data in sorted(data.items()):
        if key not in processed:
            lines.extend(format_section(key, section_data, with_labels=with_labels))
...
def format_from_database(with_labels: bool = False) -> str:
    ...
    try:
        ...
        for section in sections:
            ...
            for component in section_components:
                lines.extend(format_component(component, level=3, with_labels=with_labels))
...
def get_npl_dir(path: Optional[str]) -> Optional[Path]:
    """Determine NPL directory from path or defaults."""
    if path:
        return Path(path)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Load and format NPL definitions (default: from database)")
    parser.add_argument(
        "--path", "-p",
        type=str,
        default=None,
    ...
    # Commands that require npl_dir
    if args.sync or args.from_yaml or args.list:
        ...
        # --from-yaml: output formatted markdown from YAML files
        output = format_npl_output(data, with_labels=args.with_labels)
        print(output)
        return 0

    # Default: output from database
    output = format_from_database(with_labels=args.with_labels)
    print(output)
    return 0

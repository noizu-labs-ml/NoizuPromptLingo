# ---------------------------------------------------------------------------
# Terragrunt wrapper for the `platform/analytics` stack (platform-analytics).
# ---------------------------------------------------------------------------
# Declares its own `backend "s3"` (MinIO) in provider.tf (key
# platform/analytics/terraform.tfstate), so root.hcl stays backend-agnostic. Supply the

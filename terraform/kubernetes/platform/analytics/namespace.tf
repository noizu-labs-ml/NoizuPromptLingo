# ---------------------------------------------------------------------------
# Platform-analytics tier namespace. Holds matomo (web analytics) + growthbook (feature flags).
# Credentials managed by Infisical (syncs /analytics + /shared/* into the managed Secrets). Requires
...
locals {
  ns = kubernetes_namespace_v1.analytics.metadata[0].name

  common_labels = {
    "app.kubernetes.io/part-of"    = "noizu-platform-analytics"

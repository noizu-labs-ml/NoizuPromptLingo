# ---------------------------------------------------------------------------
# Platform-creative tier namespace. Holds the diagram + design tools (chartdb,
# drawio, excalidraw, kroki, mermaid, mydraft, penpot, plantuml, webstudio).
# Credentials managed by Infisical (/creative + /shared/* -> managed Secrets). Requires
# the infisical operator + universal-auth-credentials (set up by infra-services).
# ---------------------------------------------------------------------------
resource "kubernetes_namespace_v1" "creative" {
  metadata {
    name = var.namespace
  }
}
...
locals {
  ns = kubernetes_namespace_v1.creative.metadata[0].name

  common_labels = {
    "app.kubernetes.io/part-of"    = "noizu-platform-creative"

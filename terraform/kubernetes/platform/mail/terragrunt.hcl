# ---------------------------------------------------------------------------
# Terragrunt wrapper for the `platform/mail` stack (Postfix / Dovecot / Rspamd /
# Roundcube mail suite + its own PostgreSQL + Redis).
# ---------------------------------------------------------------------------
# Declares its own `backend "s3"` (MinIO) in provider.tf (key
# platform/mail/terraform.tfstate), so root.hcl stays backend-agnostic. Supply the
...
# stack — it bundles its own postgresql.tf + redis.tf and does NOT use the shared
# platform-init data services, so it carries no `../init` dependency. It only reads
# the root init outputs (storage class) and relies on the Infisical operator +

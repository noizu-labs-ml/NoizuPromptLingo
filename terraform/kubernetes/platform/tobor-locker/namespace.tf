resource "kubernetes_namespace_v1" "tobor_locker" {
  metadata {
    name = var.namespace
  }
}
...
locals {
  ns = kubernetes_namespace_v1.tobor_locker.metadata[0].name

  common_labels = {
    "app.kubernetes.io/part-of"    = "noizu-platform-tobor-locker"

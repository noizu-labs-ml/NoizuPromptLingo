# ---------------------------------------------------------------------------
# Terragrunt wrapper for the `platform/accounting` stack (ERPNext + Kimai).
# ---------------------------------------------------------------------------
# Declares its own `backend "s3"` (MinIO) in provider.tf (key
# platform/accounting/terraform.tfstate), so root.hcl stays backend-agnostic.
# Supply the MinIO credentials as AWS_* env vars at apply time (from init's
# outputs):
#
...
# stack — ERPNext bundles its own mariadb (erpnext.tf), so it does NOT use the
# shared platform-init data services and carries no `../init` dependency. It only

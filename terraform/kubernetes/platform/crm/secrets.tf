...
# App secrets (/crm) -> crm-app-secrets.
# Keys (from Infisical):
...
# NOTE: <APP>_DB_PASSWORD must match the value in /platform/mariadb (espocrm) or
# /platform/postgres (bottlecrm) so the shared DB role and the app agree.
resource "kubectl_manifest" "infisical_app_secrets" {
  yaml_body = yamlencode({
    apiVersion = "secrets.infisical.com/v1alpha1"
    kind       = "InfisicalSecret"
    metadata = {
      name      = "infisical-crm-secrets"
      namespace = local.ns
  ...
  depends_on = [kubernetes_namespace_v1.crm]
}

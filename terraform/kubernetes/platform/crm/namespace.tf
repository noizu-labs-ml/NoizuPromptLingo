# ---------------------------------------------------------------------------
# Platform-CRM tier namespace. Holds the self-hosted CRM apps (espocrm,
# bottlecrm). Credentials are managed by Infisical (the operator syncs them from
# /crm + /shared/* into the managed Secrets these workloads consume). Requires
...
locals {
  ns = kubernetes_namespace_v1.crm.metadata[0].name

  common_labels = {
    "app.kubernetes.io/part-of"    = "noizu-platform-crm"

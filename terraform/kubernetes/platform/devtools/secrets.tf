...
# App secrets (/devtools) -> devtools-app-secrets.
# Keys: CODE_SERVER_PASSWORD; LIVECODES_REDIS_URL (full redis DSN incl Valkey pass, db 2).
#   (Valkey password lives in /platform/valkey.)
resource "kubectl_manifest" "infisical_app_secrets" {
  yaml_body = yamlencode({
    apiVersion = "secrets.infisical.com/v1alpha1"
    kind       = "InfisicalSecret"
    metadata = {
      name      = "infisical-devtools-secrets"
      namespace = local.ns
  ...
  depends_on = [kubernetes_namespace_v1.devtools]
}

# ---------------------------------------------------------------------------
# Platform-devtools tier namespace. Holds code-server (browser IDE) + livecodes (code playground).
# Credentials managed by Infisical (syncs /devtools + /shared/* into the managed Secrets). Requires
# the infisical operator + universal-auth-credentials (set up by infra-services).
# ---------------------------------------------------------------------------
resource "kubernetes_namespace_v1" "devtools" {
  metadata {
    name = var.namespace
  }
}
...
locals {
  ns = kubernetes_namespace_v1.devtools.metadata[0].name

  common_labels = {
    "app.kubernetes.io/part-of"    = "noizu-platform-devtools"

...
# App secrets (/seo) -> seo-app-secrets.
# Keys (from Infisical): SEONAUT_DB_PASSWORD, SERPBEAR_USER, SERPBEAR_PASSWORD,
#   SERPBEAR_SECRET, SERPBEAR_API_TOKEN.
resource "kubectl_manifest" "infisical_app_secrets" {
  yaml_body = yamlencode({
    apiVersion = "secrets.infisical.com/v1alpha1"
    kind       = "InfisicalSecret"
    metadata = {
      name      = "infisical-seo-secrets"
      namespace = local.ns
  ...
  depends_on = [kubernetes_namespace_v1.seo]
}

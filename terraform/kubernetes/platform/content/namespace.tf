# ---------------------------------------------------------------------------
# Platform-content tier namespace. Holds docmost (wiki), ghost (blog),
# nextcloud (files). Credentials are managed by Infisical (the operator syncs
# them from /content + /shared/* into the managed Secrets). Requires
# the infisical operator + universal-auth-credentials (set up by infra-services).
# ---------------------------------------------------------------------------
resource "kubernetes_namespace_v1" "content" {
  metadata {
    name = var.namespace
  }
}
...
locals {
  ns = kubernetes_namespace_v1.content.metadata[0].name

  common_labels = {
    "app.kubernetes.io/part-of"    = "noizu-platform-content"

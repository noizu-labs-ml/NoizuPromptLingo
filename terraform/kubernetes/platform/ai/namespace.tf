# ---------------------------------------------------------------------------
# Platform-AI tier namespace. Consolidates the AI workloads that previously
# lived in ai-ns / qdrant-ns / vllm-ns into a single namespace.
# ---------------------------------------------------------------------------
# Credentials are managed by Infisical (the operator syncs them from /ai/* into
# the managed Secrets these workloads consume). Requires the infisical operator
# + universal-auth-credentials secret (set up by infra-services).
...
locals {
  ns = kubernetes_namespace_v1.ai.metadata[0].name

  common_labels = {
    "app.kubernetes.io/part-of"    = "noizu-platform-ai"

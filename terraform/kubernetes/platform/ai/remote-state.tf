# Read the root init module's outputs (storage class). init uses a local
# backend, so we read its state file directly. platform/ai is two levels under
# kubernetes/, so the root init module lives at ../../init.

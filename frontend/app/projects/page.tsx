...
export default function ProjectsPage() {
  ...
  return (
    <div
      className="space-y-8"
      {...cyAttrs({ cy: "projects-page", cyScope: "projects" })}
    >
      <PageHeader
        title="Projects"
      ...
      {!isLoading && projects && projects.length > 0 && (
        <div
          className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3"
          {...cyAttrs({ cy: "projects-grid", cyValue: projects.length })}
        >
          {projects.map((project) => (
            <Link
              key={project.id}
              href={`/projects/${project.id}`}
              className="block rounded-lg group focus-ring"
              {...cyAttrs({ cy: "project-card", cyId: project.id })}
            >
              <Card hoverable className="h-full flex flex-col gap-3">
                <div className="flex flex-col gap-1">
                  <h2
                    ...
                    {...cyAttrs({ cy: "project-title" })}
                  >
                    {project.title}
                  </h2>
                  <span
                    className="text-xs font-mono text-muted"
                    {...cyAttrs({ cy: "project-name" })}
                  >
                    {project.name}
                  </span>
                </div>
                ...
                <p
                  ...
                  {...cyAttrs({ cy: "project-description" })}
                >
                  {project.description}
                </p>
                ...
                <div className="flex items-center gap-2 flex-wrap pt-1 border-t border-border">
                  <span
                    className="inline-flex items-center rounded-full bg-accent/10 text-accent px-2.5 py-0.5 text-xs font-medium"
                    {...cyAttrs({ cy: "persona-count", cyValue: project.persona_count })}
                  >
                    {project.persona_count} personas
                  </span>
                  <span
                    className="inline-flex items-center rounded-full bg-accent/10 text-accent px-2.5 py-0.5 text-xs font-medium"
                    {...cyAttrs({ cy: "story-count", cyValue: project.story_count })}
                  >

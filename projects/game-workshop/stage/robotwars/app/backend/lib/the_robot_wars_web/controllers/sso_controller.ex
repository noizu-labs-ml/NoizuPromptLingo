defmodule TheRobotWarsWeb.SSOController do
  ...
  def exchange(conn, %{"code" => code}) do
    with {:ok, session_id} <- TheRobotWars.Auth.SSOCode.exchange(code),
         {:ok, session} <- TheRobotWars.Users.Sessions.get(session_id, Noizu.Context.system()),
  ...
  defp handle_sso_callback(conn, provider_type, attrs) do
    ...
    case TheRobotWars.Auth.SSO.authenticate_sso(provider_type, attrs) do
      {:ok, session} ->
        {:ok, code} = TheRobotWars.Auth.SSOCode.create(session.id)
  ...
  defp maybe_add(list, flag, name) do
    if Application.get_env(:the_robot_wars, flag), do: [name | list], else: list
  end
end

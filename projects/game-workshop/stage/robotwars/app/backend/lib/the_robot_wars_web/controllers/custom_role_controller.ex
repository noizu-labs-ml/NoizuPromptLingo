defmodule TheRobotWarsWeb.CustomRoleController do
  use TheRobotWarsWeb, :controller

  alias TheRobotWars.Schema.Organizations.CustomRole, as: RoleSchema
  alias TheRobotWars.Schema.Organizations.CustomRolePermission, as: PermSchema
  ...
  def index(conn, %{"org_id" => org_id}) do
    ...
    case Authz.authorize(user_id, "organization", org_id, "viewer") do
      {:ok, _} ->
        roles = TheRobotWars.Repo.all(from r in RoleSchema, where: r.organization_id == ^org_id and r.is_active == true)
  ...
  def create(conn, %{"org_id" => org_id, "role" => role_params}) do
    ...
    if Authz.check_permission(user_id, "organization", org_id, "organization:manage_settings") do
      ...
      case %RoleSchema{} |> RoleSchema.changeset(attrs) |> TheRobotWars.Repo.insert() do
  ...
  def show(conn, %{"org_id" => org_id, "id" => role_id}) do
    ...
    case Authz.authorize(user_id, "organization", org_id, "viewer") do
      {:ok, _} ->
        case TheRobotWars.Repo.one(from r in RoleSchema, where: r.id == ^role_id and r.organization_id == ^org_id) do
          nil -> conn |> put_status(:not_found) |> json(%{error: "Role not found"})
          role ->
            permissions = TheRobotWars.Repo.all(from p in PermSchema, where: p.role_id == ^role_id, select: p.permission)
  ...
  def update(conn, %{"org_id" => org_id, "id" => role_id, "role" => role_params}) do
    ...
    if Authz.check_permission(user_id, "organization", org_id, "organization:manage_settings") do
      case TheRobotWars.Repo.one(from r in RoleSchema, where: r.id == ^role_id and r.organization_id == ^org_id) do
        nil -> conn |> put_status(:not_found) |> json(%{error: "Role not found"})
        role ->
          case role |> RoleSchema.changeset(role_params) |> TheRobotWars.Repo.update() do
  ...
  def delete(conn, %{"org_id" => org_id, "id" => role_id}) do
    ...
    if Authz.check_permission(user_id, "organization", org_id, "organization:manage_settings") do
      case TheRobotWars.Repo.one(from r in RoleSchema, where: r.id == ^role_id and r.organization_id == ^org_id) do
        nil -> conn |> put_status(:not_found) |> json(%{error: "Role not found"})
        role ->
          case role |> Ecto.Changeset.change(is_active: false) |> TheRobotWars.Repo.update() do
  ...
  def add_permission(conn, %{"org_id" => org_id, "role_id" => role_id, "permission" => permission}) do
    ...
    if Authz.check_permission(user_id, "organization", org_id, "organization:manage_settings") do
      case %PermSchema{} |> PermSchema.changeset(%{role_id: role_id, permission: permission}) |> TheRobotWars.Repo.insert() do
        {:ok, _} ->
          permissions = TheRobotWars.Repo.all(from p in PermSchema, where: p.role_id == ^role_id, select: p.permission)
  ...
  def remove_permission(conn, %{"org_id" => org_id, "role_id" => role_id, "permission_id" => perm_id}) do
    ...
    if Authz.check_permission(user_id, "organization", org_id, "organization:manage_settings") do
      case TheRobotWars.Repo.one(from p in PermSchema, where: p.id == ^perm_id and p.role_id == ^role_id) do
        nil -> conn |> put_status(:not_found) |> json(%{error: "Permission not found"})
        perm ->
          {:ok, _} = TheRobotWars.Repo.delete(perm)

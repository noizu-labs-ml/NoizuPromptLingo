defmodule TheRobotWarsWeb.UserController do
  use TheRobotWarsWeb, :controller

  alias TheRobotWars.Guardian
  alias TheRobotWars.Users
  ...
  defp validate_update_params(user, params) do
    if params["new_password"] && params["current_password"] do
      ...
      q =
        from c in TheRobotWars.Schema.Users.Credentials.UserCredential,
  ...
  defp apply_updates(user, params, _context) do
    ...
    if map_size(updates) > 0 do
      ...
      |> TheRobotWars.Repo.update_all(set: Enum.to_list(updates))
    end

    if params["new_password"] do
      TheRobotWars.Users.Credentials.update_password(user, params["new_password"], Noizu.Context.system())
    end

defmodule TheRobotWars.Users do
  @moduledoc """
  Context for TheRobotWars.Users
  """
  alias TheRobotWars.Users.User, as: Entity
  alias TheRobotWars.Schema.Users.User, as: Schema
  use Noizu.Repo
  def_repo(entity: TheRobotWars.Users.User)
  ...
  def register(details, {:login, {email, password}} = auth, context, options) do
    ...
    email = TheRobotWars.Users.Credentials.standardize_email(email)
    ...
    with :valid <- valid_user_name?(details.user_name),
      ...
      with {:ok, name_entity} <-
             %TheRobotWars.Versioned.Names.Name{
               first: name.first,
               middle: name.middle,
               last: name.last,
           ...
           {:ok, description} <-
             %TheRobotWars.Versioned.Descriptions.Description{
               title: "Description",
           ...
           {:ok, credential} <-
             TheRobotWars.Users.Credentials.register(user_ref, auth, context, options) do
        {:ok, {user, credential}}
      end
    end
  end
  ...
  def authenticate(auth = {:login, {_email, _password}}, context, options \\ nil) do
    with {:ok, session} <- TheRobotWars.Users.Credentials.authenticate(auth, context, options) do
      {:ok, session}
    end
  end

  def by_handle(handle, context, options \\ []) do
    with record = %Schema{} <- TheRobotWars.Repo.get_by(Schema, %{handle: handle}) do
      settings = Noizu.Entity.Meta.persistence(Entity) |> hd
      {:ok, entity} = Entity.from_record(record, settings, context, options)
  ...
  def user_name_available?(user_name, _context, _options \\ nil) do
    with %{} <- TheRobotWars.Repo.get_by(Schema, %{user_name: user_name}) do
        ...
        fn
          ...
          {"name", value} ->
            case user.name do
              %Ecto.Changeset{} ->
                value =
                  TheRobotWars.Versioned.Names.change_versioned_name(user.name.data, value)

                {:name, value}

              _ ->
                value =
                  TheRobotWars.Versioned.Names.change_versioned_name(
                    user.name || %TheRobotWars.Versioned.Names.Name{},
                    value
                  )
      ...
      |> put_in(
        ...
        {:embed,
         %{
           ...
           on_cast: :update,
           on_replace: :update,
           related: TheRobotWars.Versioned.Names.Name
         }}
      )
  ...
  def valid_login?(email, password) do
    TheRobotWars.Users.Credentials.valid_login?(email, password)
  end

  def login_available?(email, context, options \\ nil) do
    TheRobotWars.Users.Credentials.login_available?(email, context, options)
  end
  ...
  defp unique_handle(handle, context, options) do
    case TheRobotWars.Users.by_handle(handle, context, options) do
      nil ->
        {:ok, handle}
      ...
      {:ok, _} ->
        Enum.reduce_while(0..999, handle, fn suffix, _acc ->
          ...
          case TheRobotWars.Users.by_handle(with_suffix, context, options) do
            nil ->
              {:halt, {:ok, with_suffix}}

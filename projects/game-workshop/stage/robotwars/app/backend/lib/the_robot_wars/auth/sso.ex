defmodule TheRobotWars.Auth.SSO do
  alias TheRobotWars.Schema.Users.User, as: UserSchema
  alias TheRobotWars.Schema.Users.Credentials.UserCredential, as: CredentialSchema
  alias TheRobotWars.Schema.Versioned.Names.Name
  import Ecto.Query, only: [from: 2]

  @provider_map %{
    oidc: &TheRobotWars.Auth.Providers.oidc/0,
    saml: &TheRobotWars.Auth.Providers.saml/0,
    google: &TheRobotWars.Auth.Providers.google/0,
    facebook: &TheRobotWars.Auth.Providers.facebook/0,
    github: &TheRobotWars.Auth.Providers.github/0,
    linkedin: &TheRobotWars.Auth.Providers.linkedin/0
  }
  ...
  def authenticate_sso(provider_type, %{email: email} = attrs) do
    ...
    {:ok, provider_id} = TheRobotWars.Auth.Providers.Provider.id(provider_ref)
    ...
    case find_user_by_email(email) do
      ...
      :not_found ->
        if Application.get_env(:the_robot_wars, :sso_require_invite, false) do
    ...
    case TheRobotWars.Repo.one(q) do
      nil ->
        %TheRobotWars.Users.Credentials.UserCredential{
          user: TheRobotWars.Users.User.ref(user.id),
  ...
  defp auto_provision_user(email, attrs, provider_ref, provider_id, provider_type, context) do
    ...
    {:ok, name} =
      TheRobotWars.EntityRepo.create(
        %TheRobotWars.Versioned.Names.Name{first: first, last: last, time_stamp: Noizu.Entity.TimeStamp.now()},
        context
      )
    ...
    {:ok, user} = TheRobotWars.Repo.insert(user_schema, on_conflict: :nothing, conflict_target: :email)

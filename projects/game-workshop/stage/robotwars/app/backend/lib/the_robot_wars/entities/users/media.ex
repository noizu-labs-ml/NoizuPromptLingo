defmodule TheRobotWars.Users.Media do
  alias TheRobotWars.Users.Media.Asset, as: Entity
  alias TheRobotWars.Schema.Users.Media.Asset, as: Schema
  use Noizu.Repo
  ...
  def list_for_user(user_id, context, options \\ []) do
    import Ecto.Query
    settings = Noizu.Entity.Meta.persistence(Entity) |> hd
    TheRobotWars.Repo.all(from m in Schema, where: m.user_id == ^user_id)
    |> Enum.map(fn record ->
      {:ok, entity} = Entity.from_record(record, settings, context, options)
      {:ok, entity} = __after_get__(entity, context, options)
      entity
